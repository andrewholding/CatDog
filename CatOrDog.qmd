...
\
The layers also need an activation function, this mimics the activation function of a neuron but for simplicity we use the 'relu' function over a more biologically accurate function. The function is very simple; if x \< 0 then the output f(x) = 0, if x \> 0 then f(x) = x. The neuron is only active if the input is positive, and then the strong the input the stronger the output.
...
\
Your final goal should be to try to set up a script that runs on biological data, a good source for data is https://www.kaggle.com/datasets/ambarish/breakhis or the simplified https://www.kaggle.com/datasets/forderation/breakhis-400x, but feel free to find your own.\
\
https://www.kaggle.com/code/azazurrehmanbutt/breakhis-images-classification-cnn
...
historyCNN_variations <- modelCNN %>% fit(
  train_datagen,
  steps_per_epoch = n_train_samples / batch_size,
  epochs = 20,
  validation_data = validation_datagen,
...
# Test the model 
  ...
  batch <- generator_next(validation_datagen_BreaKHis)
  images <- batch[[1]]
  labels <- batch[[2]]
...
for (n in 1:predict_batch_size) {
  ...
  if (label==1) {label="Benign"} else {label="Malignant"}
  if (predictions[n] > 0.5 ) {guess="Benign"} else {guess="Malignant"}
  bImage<-brick(image)
  crs(bImage)<-"+proj=longlat"
  plotRGB(bImage, colNA=0, scale=1, axes=TRUE, main=paste0("Label: ", label, ",\n P=", round(predictions[n],2), ", Prediction: ",guess))
  Sys.sleep(1) 
